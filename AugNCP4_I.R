...
dynamic_p4_II = function(n,p,alpha,seed,m,basedir,l,l2){
  ...
  sm_est_aug = ADMM_gen_block(rho, lambda, alpha, 1e-3, 1000, eles_x_aug_block, eles_y_aug_block,p)
  ...
  repeat{
    ...
    rej_setlin_raw = which(sqrt(n)*abs(delta_matlin)>=crit_aug, arr.ind = T)
    rej_setqua_raw = which(sqrt(n)*abs(delta_matqua)>=crit_aug, arr.ind = T)
    ...
    filtered_rows_lin <- apply(rej_setlin_raw, 1, function(row) has_reverse(row, rej_setlin_raw))
    filtered_rows_qua <- apply(rej_setqua_raw, 1, function(row) has_reverse(row, rej_setqua_raw))
    #filtered_mat <- mat[filtered_rows, ]
    
    rej_setlin = rej_setlin_raw[filtered_rows_lin,]
    rej_setqua = rej_setqua_raw[filtered_rows_qua,]
...
res = dynamic_p4_I(n = n, p = p,alpha = alpha, seed = seed, m = m, basedir = basedir, l = 5, l2 = 0.5)